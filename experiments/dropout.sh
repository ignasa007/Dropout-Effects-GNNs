...
depth="${depth:-4}"
bias="${bias:-true}"
# https://github.com/kedar2/FoSR/blob/1a7360c2c77c42624bdc7ffef1490a2eb0a8afd0/models/graph_model.py#L73
    ...
    for gnn in "${gnns[@]}"; do
        for dropout in "${dropouts[@]}"; do
            for drop_p in $( [[ "$dropout" == "NoDrop" ]] && echo "0.0" || echo "${drop_ps[@]}" ); do
                echo "Running: bash experiments/dropout.sh --datasets ${dataset} --gnns ${gnn} --bias ${bias} --hidden_size ${hidden_size} --depth ${depth} --gat_attention_heads ${gat_attention_heads} --graph_pooler ${graph_pooler} --dropouts ${dropout} --drop_ps ${drop_p} --learning_rate ${learning_rate} --weight_decay ${weight_decay} --n_epochs ${n_epochs} --device_index ${device_index} --total_samples ${total_samples}"
