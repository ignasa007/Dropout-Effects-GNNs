...
class TUDataset(Inductive):
    ...
    def __init__(self, name: str, config: Namespace, others: Namespace, device: torch.device):
        ...
        dataset = TUDatasetTorch(
            ...
            name=name.upper(),
            # use_node_attr=True,     # Karhadkar et al. (2022) doesn't use node attributes
            pre_transform=pre_transform,
        ).to(device)

        dataset = self.rewire(dataset.shuffle(), config=config, others=others, device=device)
        ...
        self.train_loader, self.val_loader, self.test_loader = create_loaders(
            split_dataset(dataset.shuffle(), train_split=0.8, val_split=0.1, test_split=0.1),
        ...
        super(TUDataset, self).__init__(device=device)
