...
from dataset.utils import create_loaders, CustomDataset


root = f'{root}/ZINC'
batch_size = 200    # Divisible by 1k


class SyntheticZINC(Inductive):

    def __init__(self, node_pairs_fn: str, config: Namespace, others: Namespace, device: torch.device):
        ...
        self.num_features = 1
        self.num_targets = 1
        super(SyntheticZINC, self).__init__(device=device)

    def check_args(self, others: Namespace):

        assert others.graph_pooler == 'max', f'For SyntheticZINC, the `graph_pooler` argument must be `max`.'
    ...
    def make_dataset(self, node_pairs_fn, split, size, device):
        ...
        # Create custom dataset (just to be able to use GPUs efficiently)
...
class SyntheticZINC_SD(SyntheticZINC):
    ...
    def __init__(self, config: Namespace, others: Namespace, device: torch.device, **kwargs):

        super(SyntheticZINC_SD, self).__init__(
            node_pairs_fn=self.__class__.__name__,
            config=config,
            others=others,
            device=device,
        )
    ...
    def process_args(self, others: Namespace):
        ...
        super(SyntheticZINC_SD, self).check_args(others)

        distance = float(others.syn_zinc_distance)
        assert distance >= 0
    ...
    def get_node_pair_choices(self, edge_index):
        ...
        from over_squashing.utils import compute_shortest_distances
...
class SyntheticZINC_CT(SyntheticZINC):
    ...
    def __init__(self, config: Namespace, others: Namespace, device: torch.device):
        ...
        super(SyntheticZINC_CT, self).__init__(
            node_pairs_fn=self.__class__.__name__,
            config=config,
            others=others,
            device=device,
    ...
    def process_args(self, others: Namespace):
        ...
        super(SyntheticZINC_CT, self).check_args(others)
    ...
    def get_node_pair_choices(self, edge_index):
        ...
        from over_squashing.utils import compute_commute_times
...
if __name__ == '__main__':
    ...
    for distance in distances:
        others = Namespace(distance=distance, graph_pooler='max')
        SyntheticZINC_CT(others=others, device=device)