...
class DropSens(BaseDropout):
    ...
    def __init__(self, dropout_prob: float = 0.5, others: Optional[Namespace] = None):
        ...
        self.c = others.dropsens_info_save_ratio
    ...
    def init_mapper(self, edge_index):
        ...
        if contains_self_loops(edge_index):
            warnings.warn('Degree computation in DropSens assumes absence of self-loops, \
                but the edge_index received contains some.')
    ...
    def update_mapper(self, edge_index):
    ...
    def compute_edge_dropping_probs(self, edge_index):
        ...
        if not hasattr(self, 'mapper'):
            ...
            self.update_mapper(edge_index)

        degrees = degree(edge_index[1]).int()

        # degrees[edge_index[1]]: i -> d_i
        # self.mapper[degrees[edge_index[1]]]: (i -> d_i) -> q_i
        return self.mapper[degrees[edge_index[1]].to('cpu')]
    ...
    def apply_adj_mat(self, edge_index, edge_attr=None):
        ...
        edge_mask = self.compute_edge_dropping_probs(edge_index) <= torch.rand(edge_index.size(1))
