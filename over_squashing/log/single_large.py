...
for i in tqdm(node_samples):
    ...
    fn = f'{i_dir}/subset_indices.pkl'
    if not os.path.isfile(fn):
        os.makedirs(i_dir, exist_ok=True)
        torch.save(subset, fn)
    ...
    # Save the labels of source nodes
    fn = f'{i_dir}/labels.pkl'
    if not os.path.isfile(fn):
        os.makedirs(i_dir, exist_ok=True)
        torch.save(dataset.y[subset], fn)

    edge_index, _ = subgraph(subset, dataset.edge_index, relabel_nodes=True, num_nodes=dataset.num_nodes)
    # Checked the implementation -- relabelling is such that subset[i] is relabelled as i
    x = dataset.x[subset, :]
    new_i = subset.tolist().index(i)
    ...
    for model_sample in range(1, INIT_SAMPLES*MASK_SAMPLES+1):
        if config.dropout != 'DropSens':
            ...
            save_fn = f'{i_dir}/{config.gnn}/{config.dropout}/P={config.drop_p}/C={others.dropsens_info_save_ratio}/sample={model_sample}.pkl'
