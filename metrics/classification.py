...
class Classification(Metrics):
    ...
    def __init__(self, num_targets: int, device: torch.device):

        super(Classification, self).__init__()

        if not isinstance(num_targets, int):
            raise TypeError(f'Expected `num_targets` to be an instance of `int` (got {type(num_targets)}).')
        ...
        if num_targets == 2:
            ...
            self.accuracy_fn = MulticlassAccuracy(num_targets).to(device)
            self.f1score_fn = MulticlassF1Score(num_targets).to(device)
            self.auroc_fn = MulticlassAUROC(num_targets).to(device)
        else:
            raise ValueError(f'Expected `num_targets` to be >1 (got {num_targets}).')
